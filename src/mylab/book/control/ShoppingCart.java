...
public class ShoppingCart {
    ...
    public int calculateDiscountedPrice() {
        ...
        for (Publication item : items) {
            if (item instanceof Magazine) {
                total += (int) (item.getPrice() * 0.9); // 10% 할인
            } else if (item instanceof Novel) {
                total += (int) (item.getPrice() * 0.85); // 15% 할인
            } else if (item instanceof ReferenceBook) {
                total += (int) (item.getPrice() * 0.8); // 20% 할인
            } else {
                total += item.getPrice();
            }
        }
    ...
    public void displayCart() {
        ...
        for (int i = 0; i < items.size(); i++) {
            System.out.println((i + 1) + ". " + items.get(i).getTitle() + " - " + priceFormat.format(items.get(i).getPrice()));
        }
        System.out.println("총 가격: " + priceFormat.format(calculateTotalPrice()));
